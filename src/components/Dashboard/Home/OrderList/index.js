...
function OrderList(props) {
  ...
  const {allActiveJobs} = serviceRequestReducer;

  const renderItem = (item, otherProps, theme) => (
  ...
  return (
    <View style={getStyles(theme).container}>
      ...
      <FlatList
        data={allActiveJobs}
        renderItem={(item) => renderItem(item, props, theme)}
        keyExtractor={(item) => item.id}
        ListFooterComponent={
          allActiveJobs && allActiveJobs.length >= 10 && renderFooter
        }
        style={{flex: 1}}
        ListEmptyComponent={() => (
          <View
